...
def main(num_episodes, gamma, lam, kl_targ, batch_size, env_name, monitor=False):
    ...
    while episode < num_episodes:
        ...
        with policy.sess as sess:
            on_feed_dict = {policy.obs_ph: observes,
                            policy.act_ph: actions,
                            policy.advantages_ph: advantages,
                            policy.beta_ph: policy.beta,
                            policy.eta_ph: policy.eta,
                            policy.lr_ph: policy.lr * policy.lr_multiplier}
            old_means_np, old_log_vars_np = sess.run([policy.means, policy.log_vars], feed_dict=on_feed_dict)
            on_feed_dict[policy.old_log_vars_ph] = old_log_vars_np
            on_feed_dict[policy.old_means_ph] = old_means_np
            # compute loss
            on_policy_loss = sess.run(policy.loss, feed_dict=on_feed_dict)
