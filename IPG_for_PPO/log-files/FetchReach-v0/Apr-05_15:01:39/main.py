...
from IPG_for_PPO.utils import Scaler, Logger
from IPG_for_PPO.proximal_policy_gradient import OnPolicyPPO
...
def main(num_episodes, gamma, lam, kl_targ, batch_size, env_name):
    ...
    run_policy(env, on_policy, scaler, logger, episodes=5)
    episode = 0
    ...
    with on_policy.sess as sess:
        while episode < num_episodes:
            ...
            current_buffer = []
            trajectories, episode_experiences = run_policy(env, on_policy, scaler, logger, episodes=batch_size)
            ...
            on_policy.update(loss, observes, on_actions, advantages, old_means_np, old_log_vars_np, logger)
